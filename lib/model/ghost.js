...
function Ghost(world){
    ...
    this.mode = "loop";
    this.behaviour = "waiting"; //scatter, chase, frightened, waiting, returning
    this.in_home = true;
...
Ghost.prototype.live = function(game){
    ...
    if(!this.in_home && this.behaviour != "frightened")
...
Ghost.prototype.think = function(pacman){
    ...
    else if(!this.inDecisionPoint() && !this.inHomeEntrance()){
        this.has_decided = false;
    ...
    if(this.inReturningTile() && this.behaviour == "returning"){ //ha vuelto a casa, se encierra y se pone a esperar
        this.in_home = true;
    }
}
...
Ghost.prototype.inHomeEntrance = function(){
    let tile = this.getPositionAsTile();
    if(this.logical_map[tile[0]*NCOLS + tile[1]] == 4) 
        return true;
    return false;
}

/*
Comprueba si la posición actual es ya el returning tile
*/
Ghost.prototype.inReturningTile = function(){
    let tile = this.getPositionAsTile();
    let returning_tile = [];

    if(this instanceof Blinky)
        returning_tile = [...BLINKY_RETURNING_TILE];
    else if(this instanceof Inky)
        returning_tile = [...INKY_RETURNING_TILE];
    else if(this instanceof Clyde)
        returning_tile = [...CLYDE_RETURNING_TILE];
    else if(this instanceof Pinky)
        returning_tile = [...PINKY_RETURNING_TILE];

    if(tile[0] == returning_tile[0] && tile[1] == returning_tile[1])
        return true;
    return false;
}

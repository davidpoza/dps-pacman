...
function Display(canvas) {
    ...
    */
    ...
    this.drawMap = function(image, game){
        let cols = game.world.tile_columns;
        for (let i=0; i<game.world.map.length; i++){
            let frame         = game.tile_set.frames[game.world.map[i]];
            let source_x      = frame.x;
            let source_y      = frame.y;
            let destination_x = (i % cols) * TILE_SIZE;
            let destination_y = Math.floor(i / cols  ) * TILE_SIZE;
